...
export default function CandidateContextProvider({ children }) {
  const [savedCandidates, setSavedCandidates] = useLocalStorage(
    "saved-candidates",
    []
  );

  async function createCandidate() {
    let candidate = await Promise.all([
      fetch(
    ...
    const maxTime = generateRandomNumber(
      minTime + 1,
      Math.ceil(minTime * 1.25),
      0
    );
  ...
  return (
    ...
    >
      <SaveLikedCandidates.Provider
        value={({savedCandidates, removeAllCandidates})}
      >
        {children}
      </SaveLikedCandidates.Provider>
    </GenerateCandidate.Provider>
